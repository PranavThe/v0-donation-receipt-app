...
export async function generateReceiptPDF(
  ...
  if (options?.returnBase64) {
    // Use jsPDF to generate a real PDF binary for email attachment
    const logoBase64 = await fetchLogoBase64()
    return buildJsPDF(data, logoBase64)
  }
...
// ── Fetch logo as white-canvas PNG base64 ─────────────────────────────────────
...
// ── jsPDF receipt — used only for the email attachment ────────────────────────
function buildJsPDF(data: ReceiptData, logoBase64?: string): string {
  const C = {
  ...
  // Red rule
  sd(doc, C.red); doc.setLineWidth(2); doc.line(ML, y, PW - MR, y); y += 2
  ...
  const metaCell = (cx: number, cy: number, label: string, value: string, bold: boolean) => {
    ...
    doc.setFontSize(bold ? 11 : 10); doc.setFont("helvetica", bold ? "bold" : "normal")
  ...
  // Donor
  doc.setFontSize(7.5); doc.setFont("helvetica", "normal"); st(doc, C.lite)
  doc.text("RECEIVED WITH THANKS FROM", ML, y); y += 12
  doc.setFontSize(13); doc.setFont("helvetica", "bold"); st(doc, C.dark)
  doc.text(data.donorName, ML, y); y += 14
  ...
  // Table
  const colNo = 26, colAmt = 90, hH = 22, dH = 28, fH = 26, pad = 10, tTop = y
  const amt = "$" + data.donationAmount.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })
  ...
  // Signature
  doc.setFontSize(9); doc.setFont("helvetica", "normal"); st(doc, C.mid)
  doc.text("Acknowledged with gratitude,", ML, y); y += 22
  sd(doc, C.sig); doc.setLineWidth(0.5); doc.line(ML, y, ML + 115, y); y += 7
  doc.setFontSize(10); doc.setFont("helvetica", "bold"); st(doc, C.dark)
